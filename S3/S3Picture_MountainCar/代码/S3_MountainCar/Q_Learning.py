...
def policy_f(env, scaler, featurizer, print_ep_lens):
    ...
    print("___visualize Learning Stats of the Agent____")
    visualize.plot_cost_to_go_mountain_car(env, estimator)
    visualize.plot_episode_stats(stats, smoothing_window=25)
    final_policy = greedy_policy(estimator, env.action_space.n)
    return final_policy, estimator


def q_learning(env, estimator, reward_fn, num_episodes, num_trajectory=0, discount_factor=1.0, epsilon=0.0,
               epsilon_decay=1.0, print_ep_details=False):
...
def q_learning_best_policy(env, estimator, num_episodes, discount_factor=1.0, epsilon=0.0, epsilon_decay=1.0,
    ...
    stats = visualize.EpisodeStats(episode_lengths=np.zeros(num_episodes), episode_rewards=np.zeros(num_episodes))
